...
export default function EditProductPage({
    ...
    Promise.all([
      ...
      .then(([catData, prodData]) => {
        ...
        if (prodData && !prodData.error) {
          ...
          setDiscountPrice(prodData.discountPrice ? String(prodData.discountPrice) : "");
          setStock(String(prodData.stock || 0));
          setAvailability(Boolean(prodData.availability));
          setFeatured(Boolean(prodData.featured));
          ...
          if (prodData.variants && Array.isArray(prodData.variants)) {
            const sz = Array.from(new Set(prodData.variants.map((v: any) => v.size).filter(Boolean))) as string[];
            const cl = Array.from(new Set(prodData.variants.map((v: any) => v.colour).filter(Boolean))) as string[];
            setSizes(sz);
            setColours(cl);
          }
        } else {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newImgs = data.files.map((f: any, idx: number) => ({
        imageUrl: f.url,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !sku.trim() || !categoryId || !price) {
      setError("Please fill required fields: Name, SKU, Category, and Price.");
      return;
    }
  ...
  if (loading) {
    return <div className="p-12 text-center text-xs text-neutral-400">Loading product details...</div>;
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-neutral-900">Edit Product: {name}</h1>
            <p className="text-xs text-neutral-500">
              Update inventory, pricing, images, and WhatsApp metadata.
            </p>
          </div>
        </div>
        ...
        {slug && (
          <Link
            href={`/product/${slug}`}
            target="_blank"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-neutral-100 hover:bg-neutral-200 text-neutral-800 rounded-lg text-xs font-medium"
          >
      ...
      {success && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 flex items-center gap-2">
          ...
          <span>Product updated successfully! Returning to catalogue...</span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between border-b border-neutral-100 pb-3">
            <div>
              ...
              <p className="text-[11px] text-neutral-400">
                Click &quot;Set Primary&quot; to change the main catalogue and WhatsApp preview thumbnail.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <label className="border-2 border-dashed border-neutral-300 hover:border-black rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-colors bg-neutral-50/50 hover:bg-neutral-50">
              ...
              <span className="text-xs font-semibold text-neutral-800">
                {uploading ? "Uploading files..." : "Upload New Image(s)"}
              </span>
            ...
            <div className="border border-neutral-200 rounded-xl p-4 flex flex-col justify-center space-y-2 bg-neutral-50/30">
              <span className="text-xs font-semibold text-neutral-700">Add by Direct Image URL:</span>
          ...
          {images.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
              {images.map((img, idx) => (
                ...
                >
                  <Image src={img.imageUrl} alt={`Image ${idx + 1}`} fill className="object-cover" />
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-2">
                    <button
                      ...
                      className="self-end p-1.5 bg-rose-600 text-white rounded-full hover:bg-rose-700"
                      title="Delete Image"
                    >
        ...
        <div className="flex gap-4 justify-end pt-4">
          <Link
            href="/admin/products"
            className="px-6 py-3 bg-neutral-100 hover:bg-neutral-200 text-neutral-800 rounded-xl text-xs font-semibold"
          >
            Cancel
          </Link>
          <button
